...
export function Bar({ value, max, className }: BarProps) {
  return (
    <div className="h-1.5 w-full overflow-hidden rounded-full bg-muted">
      <div
        className={cn("h-full rounded-full bg-gradient-primary transition-[width] duration-700", className)}
...
export function MetricCard({ title, value, sub, icon: Icon, iconClassName, trend }: MetricCardProps) {
  ...
  return (
    <div className="flex flex-col gap-3 rounded-xl border border-border bg-card p-5 shadow-card transition-all hover:border-primary/40 hover:shadow-glow">
      ...
      <p className="bg-gradient-hero bg-clip-text text-3xl font-bold text-transparent">{value}</p>
