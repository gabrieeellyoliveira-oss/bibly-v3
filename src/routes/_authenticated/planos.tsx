  ...
  return (
    <div className="flex flex-col rounded-2xl bg-card overflow-hidden"
      ...
      <div className="px-4 pb-2 border-b border-border flex-shrink-0">
        {meses > 1 && <p className="text-[11px] text-muted-foreground leading-none mb-0.5">{meses}x de</p>}
        ...
        <div className="grid grid-cols-2 gap-x-3 gap-y-1.5">
          {p.features.map(f => (
            <div key={f} className="flex items-start gap-1">
              <Check className="h-3 w-3 mt-0.5 shrink-0" style={{ color: p.cor }} />
              <span className="text-[11px] text-foreground/80 leading-snug">{f}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="px-4 pb-2">
        <button onClick={() => setOpen(v => !v)}
          className="flex items-center justify-between w-full text-[11px] font-semibold text-muted-foreground hover:text-foreground transition-colors py-1.5 border-t border-border">
        ...
        {open && (
          <ul className="space-y-1 mt-1">
            {p.integracoes.map(i => (
              <li key={i} className="text-[11px] text-foreground/70 leading-snug">• {i}</li>
            ))}
          </ul>
        )}
      </div>
